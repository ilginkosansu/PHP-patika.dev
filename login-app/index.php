...
include 'fonksiyon/helper.php';
    ...
    //session_start();
    if(!isset($_SESSION['login']) || $_SESSION['login'] == false) {//login adında bir oturum bilgisi başlatırız
        header('Location:login.php'); //oturum açılmamışsa login sayfasına yönlendir
    }
    //oturum açtıysak o bilgileri file_get_content ile çekebiliriz, textarea içerisinde yazdırabileceğimiz değişken !!session ile karıştırma!
    $hakkimda = file_get_contents("db/".sessionDegeri('username').'.txt');
...
<div  class="container d-flex align-items-center justify-content-center">
    <div class="card <?= cookieDegeri('color') ? cookieDegeri('color') : 'bg-dark'; ?>" style="width: 18rem;">
        ...
        <div class="card-body">
            <h5 class="card-title text-warning"><?= sessionDegeri('kullanici_adi') ?></h5>
            <h6 class="card-subtitle mb-2 text-muted"><?= sessionDegeri('eposta') ?></h6>
            <?php
                if(getDegeri('islem') == 'true') { ///!!!!!!
                     echo "<div class='alert alert-success'> İşlem Başarılı!</div> ";
                }
                elseif (getDegeri('islem') == 'false'){ //sadece else komutunda sayfa açıldığı gibi işlem başarısız uyarısı geliyordu
                    echo "<div class='alert alert-danger'> İşlem Başarısız!</div> ";
                }
            ?>
            <form action="islem.php?islem=hakkimda" method="post">
                <textarea class="form-control <?= cookieDegeri('color') ? cookieDegeri('color') : 'bg-dark'; ?> text-success" name="hakkimda" id="" cols="30" rows="10"><?= htmlspecialchars_decode($hakkimda)  ?></textarea>
        ...
        <div class="card-footer bg-info d-flex align-items-center justify-content-between">
            ...
            <a href="islem.php?islem=renk&color=bg-dark" class="btn btn-sm btn-dark">Dark Mod</a> <!--iki get degeri tanımlayabiliyoruz-->
        </div>
    </div>
</div>
</body>
